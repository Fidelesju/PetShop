...
namespace PetShop_backend.PetShop.Bussiness.CoreServices
{
    public class FileManagementService : IFileManagementService
    {
      ...
      /*   public async Task<string> StoreFileOnLocal(FileCreateRequestModel file, string folder)
        {
            string fileName;
            ...
            fullImagePath = Path.Combine(UploadsFolder, folder, fileName);
            webPath = $"{folder}/{fileName}";
            if (!Directory.Exists(UploadsFolder + "/" + folder))
            {
                try
                {
                    Directory.CreateDirectory(UploadsFolder + "/" + folder);
                }
            ...
            try
            {
                stream = File.Create(fullImagePath);
